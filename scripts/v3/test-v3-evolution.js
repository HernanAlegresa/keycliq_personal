/**
 * KeyScan V3 - Evolution Test Script
 * Tests the V3 optimized evolution logic
 */

import { analyzeKeyWithHybridBalancedAI } from '../../app/lib/ai/v3/multimodal-keyscan-v3.server.js';
import fs from 'fs';
...
const TEST_CONFIG = {
  ...
  resultsPath: 'tests/results/v3',
  version: 'V3 (Evolution Logic)'
};
...
async function runSingleTest(runNumber, allKeys) {
  console.log(`\n🧪 Running V3 Test ${runNumber}/${TEST_CONFIG.totalRuns}`);
  ...
  try {
    ...
    if (queryResult.success) {
      ...
      return {
        ...
        accuracy,
        passed: accuracy >= 90 // V3 threshold (higher than V2)
      };
    } else {
...
async function runV3Tests() {
  ...
  const resultsFile = path.join(TEST_CONFIG.resultsPath, `v3-test-summary-${timestamp}.json`);
...
// Run tests
runV3Tests().catch(console.error);
