/**
 * KeyScan V2 - Staging Test Script
 * Tests the current staging logic (V2)
 */

import { analyzeKeyWithHybridBalancedAI } from '../../app/lib/ai/v2/multimodal-keyscan-v2.server.js';
import fs from 'fs';
...
async function runSingleTest(runNumber, allKeys) {
  console.log(`\n🧪 Running V2 Test ${runNumber}/${TEST_CONFIG.totalRuns}`);
  ...
  try {
    ...
    if (queryResult.success) {
      ...
      return {
        ...
        accuracy,
        passed: accuracy >= 80 // V2 threshold
      };
    } else {
...
async function runV2Tests() {
  ...
  const resultsFile = path.join(TEST_CONFIG.resultsPath, `v2-test-summary-${timestamp}.json`);
