...
/**
 * KeyScan V4 Corrected Test - Real Inventory Testing
...
import { V4TestingSuiteCorrected } from './v4-testing-suite-individual.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

class V4CorrectedTestRunner {
  constructor() {
    this.baseOutputDir = path.join(__dirname, '../../../tests/results/individual-tests/v4');
    this.testSuite = new V4TestingSuiteCorrected();
  }
  ...
  async runCorrectedTest() {
    ...
    console.log('🚀 Starting KeyScan V4 Individual Test...');
    console.log('📋 Real inventory testing with proper same-key-different-image cases');
    console.log(`📁 Results will be saved in tests/results/individual-tests/v4/test-${testNumber}/`);
    ...
    try {
      ...
      console.log('\n🎉 V4 Corrected Test Completed Successfully!');
      console.log('\n📊 Final Results:');
      ...
      console.log('\n✅ V4 Corrected Testing completed successfully!');

    } catch (error) {
      console.error('❌ An error occurred during V4 corrected testing:', error);
      process.exit(1);
    }
  }
...
<head>
    ...
    <title>KeyScan V4 Corrected Test Report</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🔍 KeyScan V4 Corrected Test Report</h1>
            ...
            <div class="section">
                ...
                <div class="config-grid">
                    ...
                    <div class="config-item">
                        ...
                        <div class="value">${configuration.algorithm}</div>
                    </div>
                    <div class="config-item">
                        <div class="label">Features</div>
                        <div class="value">${configuration.features}</div>
                    </div>
                    <div class="config-item">
                        <div class="label">Preprocessing</div>
                        <div class="value">${configuration.preprocessing}</div>
                    </div>
                </div>
            </div>
...
if (import.meta.url === `file://${process.argv[1]}` || import.meta.url.endsWith('run-v4-individual.js')) {
  const runner = new V4CorrectedTestRunner();
