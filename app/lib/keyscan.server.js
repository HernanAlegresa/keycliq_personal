 ...
 * @param {Object} config - Configuraci├│n de KeyScan V3
...
export async function processKeyImageV3(imageDataURL, inventory = [], config = {}) {
  ...
  // Inicializar KeyScan V3 con configuraci├│n CALIBRADA (basada en testing exitoso)
  // DTW banda 8% + penalizaci├│n cuadr├ítica + notches permisivos + varianza
  ...
  const keyScan = new ProductionKeyScanV3({
    matching: {
      thresholds: {
        ...
        T_possible: parseFloat(process.env.KEYSCAN_THRESHOLD_POSSIBLE || '0.70'), // POSSIBLE: casos l├¡mite (0.70-0.81)
        delta: parseFloat(process.env.KEYSCAN_THRESHOLD_DELTA || '0.15')         // Margen desambiguaci├│n (15%)
      },
      weights: {
        ...
        shape: parseFloat(process.env.KEYSCAN_WEIGHT_SHAPE || '0.08')       // Shape: se├▒al secundaria
      },
      shapeVeto: {
 ...
 * @param {string} imageDataURL - Data URL de la imagen
 * @returns {Promise<Object>} Features extra├¡das
 */
