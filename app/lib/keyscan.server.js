...
/**
 * Procesa una imagen con KeyScan V5 ModelAI (Reconstructed)
 * @param {string} imageDataURL - Data URL de la imagen
 * @param {Array} inventory - Inventario del usuario con signatures
 ...
 * @returns {Promise<Object>} Resultado del procesamiento
 */
export async function processKeyImageV5ModelAI(imageDataURL, inventory = [], userId = null, config = {}) {
  try {
    const startTime = Date.now();
    ...
    console.log('🔍 KeyScan V5 ModelAI - Starting AI analysis...');
    ...
    const querySignature = analysisResult.signature;
    console.log('✅ V5 ModelAI signature generated successfully');
    ...
    if (inventory && inventory.length > 0) {
      ...
      console.log(`📊 V5 ModelAI Decision: ${decision.type}`);
    ...
    console.error('Error in processKeyImageV5ModelAI:', error);
    return {
      success: false,
      error: error.message,
      message: `V5 ModelAI Processing error: ${error.message}`,
      processingTime: 0
    };
  }
}
...
/**
 * Extrae signature de una imagen con KeyScan V5 ModelAI
 * @param {string} imageDataURL - Data URL de la imagen
 * @returns {Promise<Object>} Signature extraída
 */
export async function extractSignatureV5ModelAI(imageDataURL) {
  try {
    // Convertir dataURL a Buffer
    const { data: imageBuffer } = dataUrlToBinary(imageDataURL);
    ...
    console.error('Error in extractSignatureV5ModelAI:', error);
    return {
      success: false,
      error: error.message,
      message: `V5 ModelAI Signature extraction error: ${error.message}`
    };
  }
}
